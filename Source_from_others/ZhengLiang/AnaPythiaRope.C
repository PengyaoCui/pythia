...
//=============================================================================
    ...
    for (int i=0; i<event.size(); i++) if (event[i].isFinal() &&
      ...
      double dEtaAbs = TMath::Abs(event[i].eta()); 
			...
			if(dRapAbs<2){
				if(event[i].idAbs()==310)
					hKS_pT->Fill(event[i].pT());

				if(event[i].idAbs()==3122)
					hLambda_pT->Fill(event[i].pT());

				if(event[i].idAbs()==3312)
					hXi_pT->Fill(event[i].pT());
			}

			if(event[i].isCharged())
				hChEta_Inel->Fill(event[i].eta());
      ...
      if ( (!bKshort) && (!bKCh) && (!bLambda) && (!bProton)
				 && (!bPiCh) && (!bPhi) && (!bOmega) && (!bCascade)	) continue;
