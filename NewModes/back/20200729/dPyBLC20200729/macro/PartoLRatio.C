...
void PartoLRatio(TString sType = "Xi"){
  TString sCone = "Inclusive";
  TString sPath[] = {"../", "../", "../", "../"};
  ...
  //TString sL[] = {Form("hLambda_Jet10_%s04", sCone.Data())};
  //TString sL0[] = {Form("hAntiLa_Jet10_%s04", sCone.Data())};
  //TString sP[] = {Form("h%sNeg_Jet10_%s04", sType.Data(), sCone.Data())};
  //TString sA[] = {Form("h%sPos_Jet10_%s04", sType.Data(), sCone.Data())};
  TString sL[] = {Form("hLambda")};
  TString sL0[] = {Form("hAntiLa")};
  TString sP[] = {Form("h%sNeg", sType.Data())};
  TString sA[] = {Form("h%sPos", sType.Data())};
  
  TString sName = "XLRatio";
  Double_t XMin = 0.; Double_t XMax = 8; Double_t YMin = 0.; Double_t YMax = 0.5;
  Double_t dBin[] = {0.6, 1.0, 1.6, 2.4, 3.2, 5.0, 8, 12., 16};
  auto sLatex = (Form("%s #%s to #Lambda ratio in pp at #sqrt{s} = 13 TeV", sCone.Data(), sType.Data()));
  ...
  SetFrame(h, "#it{p}_{T} (GeV/#it{c})", Form("#%s/#Lambda", sType.Data()));
  DrawHisto(h, cLine[0], sMark[0], "same");
  
  auto f0 = TFile::Open("./result/FinalSpect_ThisAna.root", "read");
  auto l = (TList*)f0->Get(Form("%s_toLRatio", sType.Data()));
  f0->Close();
  ...
  gStyle->SetOptStat("");
  CanvasEnd(can);
  can->SaveAs(Form("./figure/%s_toLRatio_wDiffCRMode_%s.eps", sType.Data(), sCone.Data()));
  can->SaveAs(Form("./figure/%s_toLRatio_wDiffCRMode_%s.png", sType.Data(), sCone.Data()));
  return;
}
