...
void  DrawCones(TString sType = "Lambda_sum", Int_t nCent = 0);
void  DrawCents(TString sType = "Lambda_sum", Int_t nCone = 0);

void dataPartoKRatio_pPb(TString sType = "Lambda_sum", Bool_t IsDiffCone = kTRUE){
//=============================================================================

  auto f = TFile::Open("./data/Data/pPb.root", "read");
  TList* l[4];
  l[0]= (TList*)f->Get(Form("%s_toKRatio_0100", sType.Data()));
  l[1]= (TList*)f->Get(Form("%s_toKRatio_010", sType.Data()));
  l[2]= (TList*)f->Get(Form("%s_toKRatio_1040", sType.Data()));
  l[3]= (TList*)f->Get(Form("%s_toKRatio_40100", sType.Data()));
  f->Close();
  for(Int_t i = 0; i<4; i++){
...
void  DrawCones(TString sType = "Kshort", Int_t nCent = 0)
{
  ...
  if(sType == "Lambda_sum") tex->DrawLatex(0.16, 0.82, Form("#frac{#Lambda + #bar{#Lambda}}{2 K^{0}_{S}} %s", sCent[nCent].Data()));
  if(sType == "Xi" || sType == "Omega") tex->DrawLatex(0.16, 0.82, Form("#frac{#%s^{-} + #bar{#%s}^{-}}{2K^{0}_{S}} %s", sType.Data(), sType.Data(), sCent[nCent].Data()));
  
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toKRatioData_Cent%d.eps", sType.Data(), nCent));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toKRatioData_Cent%d.pdf", sType.Data(), nCent));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toKRatioData_Cent%d.png", sType.Data(), nCent));
  CanvasEnd(can);
...
void  DrawCents(TString sType = "Kshort", Int_t nCone = 0)
{
  ...
  tex->SetTextSizePixels(24);
  tex->DrawLatex(0.16, 0.92, "p-Pb at #sqrt{#it{s}_{NN}} = 5.02 TeV");
  if(sType == "Lambda_sum") tex->DrawLatex(0.16, 0.82, Form("#frac{#Lambda + #bar{#Lambda}}{2 K^{0}_{S}} %s", sCone[nCone].Data()));
  if(sType == "Xi" || sType == "Omega") tex->DrawLatex(0.16, 0.82, Form("#frac{#%s^{-} + #bar{#%s}^{-}}{2K^{0}_{S}} %s", sType.Data(), sType.Data(), sCone[nCone].Data()));
  
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toKRatioData_Cone%d.eps", sType.Data(), nCone));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toKRatioData_Cone%d.pdf", sType.Data(), nCone));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toKRatioData_Cone%d.png", sType.Data(), nCone));
  CanvasEnd(can);
