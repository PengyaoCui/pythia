...
void dataPartoLRatio_pPb(TString sType = "Omega", Bool_t IsDiffCone = kTRUE){
//=============================================================================
...
void  DrawCones(TString sType = "Omega", Int_t nCent = 0)
{
  ...
  if(sType == "Xi" || sType == "Omega") tex->DrawLatex(0.16, 0.82, Form("#frac{#%s^{-} + #bar{#%s}^{-}}{#Lambda + #bar{#Lambda}} %s", sType.Data(), sType.Data(), sCent[nCent].Data()));
  
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toLRatioData_Cent%d.eps", sType.Data(), nCent));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toLRatioData_Cent%d.pdf", sType.Data(), nCent));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toLRatioData_Cent%d.png", sType.Data(), nCent));
  CanvasEnd(can);
...
void  DrawCents(TString sType = "Omega", Int_t nCone = 0)
{
  ...
  tex->SetTextSizePixels(24);
  tex->DrawLatex(0.16, 0.92, "p-Pb at #sqrt{#it{s}_{NN}} = 5.02 TeV");
  if(sType == "Xi" || sType == "Omega") tex->DrawLatex(0.16, 0.82, Form("#frac{#%s^{-} + #bar{#%s}^{-}}{#Lambda + #bar{#Lambda}} %s", sType.Data(), sType.Data(), sCone[nCone].Data()));
  
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toLRatioData_Cone%d.eps", sType.Data(), nCone));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toLRatioData_Cone%d.pdf", sType.Data(), nCone));
  can->SaveAs(Form("./figure/pPb5d02TeV_%s_toLRatioData_Cone%d.png", sType.Data(), nCone));
  CanvasEnd(can);
